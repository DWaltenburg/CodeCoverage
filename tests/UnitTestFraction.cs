...
public class UnitTestFraction
{
    ...
    [Theory]
    [InlineData(2, 6, 3, 4, -5, 12)]
    public void TestMinusFraction(int a, int b, int c, int d, int e, int f)
    {
        // to do 
        Fraction f1 = new(a, b);
        Fraction f2 = new(c, d);
        ...
        Assert.Equal(new Fraction(e, f), res);
    }

    [Theory]
    [InlineData(-2, 10, 3, -6, 10)]
    public void TestMultiplyInteger(int a, int b, int c, int d, int e)
    {
        // to do
        Fraction f1 = new(a, b);

        var res = f1.Multiply(c);

        Assert.Equal(new Fraction(d, e), res);
    }
